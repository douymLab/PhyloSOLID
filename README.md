...
After running `plot_circos()`, the following files are generated in the output directory:

| File | Description |
|------|-------------|
| `No_target.circle_tree_output_as_point.*.svg/pdf` | Main circular plot (no embedded legend) |
...
| `df_muts_corresponding_to_ordered_tiplabels_by_anticlockwise.txt` | Ordered cell IDs with their top mutations |
| `df_flipping_count_for_each_mut.txt` | Per-mutation flip statistics |
| `df_total_flipping_count.txt` | Global flip statistics |
| `CNVtree_data_clone_order_tree.rds` | Tree structure data for further analysis |

### Interactive Inspection Platform

After generating the circos plot, you can upload the results to the **interactive inspection platform** for manual review and quality control:

👉 **https://phylosolid.westlake.edu.cn/home**

All files required for upload are located in the output directory generated by `plot_circos()`. Upload the files according to the platform's interface:

| Platform Field | Corresponding File |
|----------------|-------------------|
| Binary matrix for the circos heatmap layer | `ordered_metadata_for_heatmap.txt` |
| Ordered cell IDs with their top mutations (anti-clockwise) | `df_muts_corresponding_to_ordered_tiplabels_by_anticlockwise.txt` |
| Circos plot of the phylogenetic tree (SVG format) | `No_target.circle_tree_output_as_point.*.svg` |
| Legend figure for circos plot (SVG format) | `legend_components.circos_annotation.svg` |
| Global flip statistics | `df_total_flipping_count.txt` |
| Per-mutation flip statistics | `df_flipping_count_for_each_mut.txt` |
